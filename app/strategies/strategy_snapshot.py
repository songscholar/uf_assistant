"""
UF Stock Assistant — Strategy Snapshot Resolver

Resolves stored strategy DB rows into backtest-ready snapshot dicts.
Adapted from QuantDinger: uses SQLAlchemy ORM instead of raw SQL, no user_id.
"""
...
logger = get_logger("app.strategies.strategy_snapshot")
...
class StrategySnapshotResolver:
    ...
    def resolve(
        ...
        """Load strategy from DB and return a backtest-ready snapshot dict."""
        ...
        return self._build_snapshot(strategy_dict, override_config)
    ...
    def _build_snapshot(
        ...
        """Build a backtest-ready snapshot from a strategy dict."""
        if not strategy:
        ...
        initial_capital = self._to_float(
            override.get("initialCapital", trading_config.get("initial_capital", strategy.get("initial_capital", 10000))),
            10000.0,
        )
        ...
        if commission_raw is None or commission_raw == "":
            commission_raw = 0.05 if is_script else 0
    ...
    def _build_strategy_config(self, trading_config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "position": {
                "entryPct": self._percent_to_ratio(
                    tc.get("entry_pct") if tc.get("entry_pct") is not None else 100
                ),
            },
            "scale": {
                "trendAdd": {
    ...
    @staticmethod
    def _percent_to_ratio(value: Any, default: float = 0.0) -> float:
        try:
            raw = float(value)
        except Exception:
            raw = float(default or 0.0)
        if raw <= 0:
            return 0.0
        if raw > 100:
            raw = 100.0
