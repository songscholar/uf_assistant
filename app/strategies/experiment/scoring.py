"""
UF Stock Assistant — Strategy Scoring Service

Multi-factor scoring for backtest results. Converts raw metrics into
comparable 0-100 scores with grade ratings.
...
class StrategyScoringService:
    ...
    def score_result(
        ...
        *,
        ...
        """
        Score a backtest result using 7 weighted factors.

        Args:
            result: Backtest output dict with keys like totalReturn, sharpeRatio, etc.
            regime: Optional regime detection result for regime-fit scoring.

        Returns:
            Scoring result with overallScore, grade, components, and summary.
        """
        result = result or {}
        ...
        return {
            ...
            "summary": {
                ...
                "riskAdjustedReturn": round(
                    (components["sharpeScore"] + components["drawdownScore"]) / 2.0, 2
                ),
                "consistency": round(
                    (components["stabilityScore"] + components["winRateScore"]) / 2.0, 2
                ),
            },
        }

    def rank_results(self, items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Rank scored results by overallScore descending."""
        ranked = list(items)
        ranked.sort(
    ...
    def _estimate_regime_fit(
        self, regime: dict[str, Any], components: dict[str, float]
    ) -> float:
        """Estimate how well the strategy fits the detected market regime."""
        ...
        if regime_key == "range_compression":
            return min(
                100.0, components["winRateScore"] * 0.6 + components["stabilityScore"] * 0.4
            )
        if regime_key == "high_volatility":
            return min(
                100.0, components["drawdownScore"] * 0.6 + components["profitFactorScore"] * 0.4
            )
    ...
    def _stability_score(equity_curve: list[dict[str, Any]]) -> float:
        """Score based on monotonicity of equity curve."""
    ...
    def _bounded_score(value: float, *, floor: float, ceiling: float) -> float:
        """Linear score from floor (0) to ceiling (100)."""
        if ceiling <= floor:
            return 50.0
    ...
    def _inverse_score(value: float, *, floor: float, ceiling: float) -> float:
        """Inverse linear score: lower value = higher score."""
        if ceiling <= floor:
            return 50.0
        ratio = (value - floor) / (ceiling - floor)
