"""
UF Stock Assistant — Experiment LLM Prompts

Prompt construction and response parsing for the AI experiment pipeline.
Adapted from QuantDinger: uses UF's IndicatorParamsParser and logger.
"""
...
logger = get_logger("app.strategies.experiment.prompts")
...
def parse_llm_candidates(raw_text: str) -> list[dict[str, Any]]:
    ...
    logger.warning("failed_to_parse_llm_candidates")
    return []


def _normalize_candidate(raw: dict[str, Any]) -> dict[str, Any]:
    """Ensure candidate has the expected structure with clamped values."""
