...
模拟交易下单区域新增实时行情展示、涨跌停价格校验、智能可买/可卖数量提示。

### 修改文件

| 文件 | 改动 |
|------|------|
| `frontend/src/pages/TradingPage.tsx` | 右侧行情面板（名称/现价/涨跌幅/今开/最高/最低/涨停/跌停/成交量/成交额/市盈率/总市值）；价格输入保留两位小数；限价委托校验（不能超涨停、不能低跌停）；买入时按可用资金计算可买数量，卖出时按持仓计算可卖数量；下单成功后自动刷新资产和持仓 |

### 技术细节

- **行情获取**：输入股票代码后 500ms debounce 调用 `stockApi.getRealtime(symbol)`
- **数量计算**：买入 `Math.floor(available_cash / price / 100) * 100`，卖出 `Math.floor(quantity / 100) * 100`
- **价格校验**：`limit_up`、`limit_down` 来自行情 API，限价单提交前拦截
- **布局**：下单表单（左侧 max-w-md）+ 行情面板（右侧 flex-1），仅模拟交易模式展示
